...
KOKKOS_INLINE_FUNCTION void cmpflx(const HydroState& qgdnv, HydroState& flux,
...
KOKKOS_INLINE_FUNCTION void riemann_approx(const HydroState& qleft,
                                           ...
                                           HydroState& qgdnv, HydroState& flux,
                                           const HydroParams& params) {
  real_t gamma0 = params.settings.gamma0;
  ...
  real_t wl = SQRT(cl);
  real_t wr = SQRT(cr);
  real_t pstar = fmax(((wr * pl + wl * pr) + wl * wr * (ul - ur)) / (wl + wr),
                      (real_t)ZERO_F);
  real_t pold = pstar;
  real_t conv = ONE_F;
  ...
  real_t rstar = fmax((real_t)(ro / (ONE_F + ro * (po - pstar) / (wo * wo))),
                      (real_t)(smallr));
  ...
  if (sgnm > ZERO_F) {
    qgdnv[IV] = qleft[IV];
    if (std::is_same<HydroState, HydroState3d>::value) qgdnv[IW] = qleft[IW];
  } else {
    qgdnv[IV] = qright[IV];
    if (std::is_same<HydroState, HydroState3d>::value) qgdnv[IW] = qright[IW];
  }
 ...
 * See section 10.5.1, equation 10.43 which gives the expression of S+ denoted
 * here as cmax.
 *
 * @param[in] qleft  : input left  state (primitive variables)
 * @param[in] qright : input right state (primitive variables)
...
KOKKOS_INLINE_FUNCTION void riemann_llf(const HydroState& qleft,
  ...
  uright[IP] =
      qright[IP] * entho + HALF_F * qright[ID] * qright[IU] * qright[IU];
  ...
  for (int nVar = 0; nVar < HYDRO_2D_NBVAR; nVar++) {
    flux[nVar] = HALF_F * (fleft[nVar] + fright[nVar] -
                           cmax * (uright[nVar] - uleft[nVar]));
  }
  if (std::is_same<HydroState, HydroState3d>::value) {
    flux[IW] =
        HALF_F * (fleft[IW] + fright[IW] - cmax * (uright[IW] - uleft[IW]));
  }
}  // riemann_llf
 ...
 */
template <class HydroState>
KOKKOS_INLINE_FUNCTION void riemann_slau(const HydroState& qleft,
                                         const HydroState& qright,
                                         HydroState& qgdnv, HydroState& flux,
                                         const HydroParams& params) {
  UNUSED(qgdnv);
...
void riemann_hydro(const HydroState2d& qleft, const HydroState2d& qright,
                   HydroState2d& qgdnv, HydroState2d& flux,
                   const HydroParams& params) {
  ...
  } else if (params.riemannSolverType == RIEMANN_SLAU) {
    riemann_slau<HydroState2d>(qleft, qright, qgdnv, flux, params);
  }
 ...
 */
KOKKOS_INLINE_FUNCTION
void riemann_hydro(const HydroState3d& qleft, const HydroState3d& qright,
                   HydroState3d& qgdnv, HydroState3d& flux,
                   const HydroParams& params) {
  if (params.riemannSolverType == RIEMANN_APPROX) {
